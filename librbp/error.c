...
static int warning_stream_inited = 0;
...
static void ensure_warning_stream_inited(void) {
    if (warning_stream_inited == 0) {
        warning_stream = stderr;
        warning_stream_inited = 1;
    }
}
...
void warning(const char * fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    ensure_warning_stream_inited();
    if (warning_stream != NULL) {
        fprintf(warning_stream, "WARNING: ");
        vfprintf(warning_stream, fmt, ap);
        if (fmt[strlen(fmt) - 1] != '\n')
